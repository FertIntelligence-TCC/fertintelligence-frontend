...
const AddRangePopover = ({ type, unit, currentCount, onAdd }: { type: "P" | "K", unit: string, currentCount: number, onAdd: (label: string, operator: "less" | "between" | "more") => void }) => {
    ...
    return (
        <PopoverRoot positioning={{ placement: "right" }}>
            ...
            <PopoverContent width="320px" bg="white" _dark={{ bg: "gray.800", borderColor: "gray.600" }} borderRadius="md" shadow="lg" borderWidth="1px">
                <PopoverArrow />
                <PopoverHeader fontWeight="bold" _dark={{ color: "white" }}>Novo intervalo ({type}, {unit})</PopoverHeader>
                ...
                <PopoverBody>
                    <VStack gap={3}>
                        ...
                        <HStack>
                            <Input placeholder={`Valor 1 (${unit})`} type="number" value={n1} onChange={(e) => setN1(e.target.value)} {...commonFieldStyles} />
                            {mode === "between" && (<Input placeholder={`Valor 2 (${unit})`} type="number" value={n2} onChange={(e) => setN2(e.target.value)} {...commonFieldStyles} />)}
                        </HStack>
...
export default function NutrientTableSection({ form, onFormChange, readOnly }: Props) {
    const formatCoverageHeader = (label: string) =>
        /kg\/ha/i.test(label) ? label : `${label} (kg/ha)`;
    ...
    return (
        <Box mt={4} borderTopWidth="2px" pt={4} _dark={{ borderColor: "gray.600" }}>
            <Flex justify="space-between" align="center" mb={3}><Heading size="md" _dark={{ color: "white" }}>Tabela de doses de N, P e K</Heading></Flex>
                ...
                <Table.Root size="sm" variant="outline">
                    <Table.Header>
                        <Table.Row>
                            ...
                            {form.coberturaLabels.map((lbl, idx) => (<Table.ColumnHeader key={idx} minW="100px" {...headerCellStyles}>{formatCoverageHeader(lbl)}</Table.ColumnHeader>))}
                            <Table.ColumnHeader minW="140px" {...headerCellStyles}>
                                {!readOnly && (
                                    <HStack gap={1}>
                                        <Button size="xs" colorScheme="blue" variant="solid" onClick={addCoverageColumn} title="Adicionar cobertura (kg/ha)">(+) Cob.</Button>
                                        <Button size="xs" colorScheme="red" variant="outline" onClick={removeCoverageColumn} disabled={form.coberturaLabels.length <= 1} title="Remover última cobertura (kg/ha)">(-) Cob.</Button>
                                    </HStack>
                                )}
                            </Table.ColumnHeader>
                        </Table.Row>
                    ...
                    <Table.Body>
                        ...
                        <Table.Row bg="blue.50" _dark={{ bg: "whiteAlpha.100" }}>
                            ...
                            <Table.Cell><Input size="sm" type="number" placeholder="kg/ha" {...commonFieldStyles} value={form.plantioN} onChange={(e) => updateRowValue("N", null, "plantio", e.target.value)} readOnly={readOnly} /></Table.Cell>
                            {form.coberturasN.map((val, idx) => (<Table.Cell key={idx}><Input size="sm" type="number" placeholder="kg/ha" {...commonFieldStyles} value={val} onChange={(e) => updateRowValue("N", null, idx, e.target.value)} readOnly={readOnly} /></Table.Cell>))}
                            <Table.Cell bg="transparent" />
                        </Table.Row>

                        {/* Fósforo */}
                        <Table.Row><Table.Cell colSpan={10} {...sectionHeaderStyles}>Fósforo (P) disponível em mg/dm³</Table.Cell></Table.Row>
                        {form.faixasP.map((row) => (
                                ...
                                <Table.Cell><Input size="sm" type="number" placeholder="kg/ha" {...commonFieldStyles} value={row.plantio} onChange={(e) => updateRowValue("P", row.id, "plantio", e.target.value)} readOnly={readOnly} /></Table.Cell>
                                {row.coberturas.map((cell, idx) => (<Table.Cell key={idx}><Input size="sm" type="number" placeholder="kg/ha" {...commonFieldStyles} value={cell.value} onChange={(e) => updateRowValue("P", row.id, idx, e.target.value)} readOnly={readOnly} /></Table.Cell>))}
                            ...
                            <Table.Row>
                                <Table.Cell colSpan={10} p={3}>
                                    <VStack align="start" gap={2}>
                                        <AddRangePopover type="P" unit="mg/dm³" currentCount={form.faixasP.length} onAdd={(lbl, op) => addRangeRow("P", lbl, op)} />
                        ...
                        {/* Potássio */}
                        <Table.Row><Table.Cell colSpan={10} {...sectionHeaderStyles}>Potássio (K) trocável em mmolc/dm³</Table.Cell></Table.Row>
                        {form.faixasK.map((row) => (
                                <Table.Row key={row.id} _dark={{ bg: "transparent" }}>
                                <Table.Cell fontSize="sm" _dark={{ color: "gray.200" }}>{row.label}</Table.Cell>
                                <Table.Cell><Input size="sm" type="number" placeholder="kg/ha" {...commonFieldStyles} value={row.plantio} onChange={(e) => updateRowValue("K", row.id, "plantio", e.target.value)} readOnly={readOnly} /></Table.Cell>
                                {row.coberturas.map((cell, idx) => (<Table.Cell key={idx}><Input size="sm" type="number" placeholder="kg/ha" {...commonFieldStyles} value={cell.value} onChange={(e) => updateRowValue("K", row.id, idx, e.target.value)} readOnly={readOnly} /></Table.Cell>))}
                                <Table.Cell bg="transparent"></Table.Cell>
                                </Table.Row>
                        ))}
                        {!readOnly && (
                            <Table.Row>
                                <Table.Cell colSpan={10} p={3}>
                                    <VStack align="start" gap={2}>
                                        <AddRangePopover type="K" unit="mmolc/dm³" currentCount={form.faixasK.length} onAdd={(lbl, op) => addRangeRow("K", lbl, op)} />
