...
const PropertyFormDialog = ({
  ...
  const { data: plots = [] } = useQuery({
    ...
    queryFn: () =>
      propertyId ? getPlotsByProperty(propertyId) : Promise.resolve([]),
    enabled: !!propertyId && isOpen,
  });

  const createPlotMutation = useMutation({
    mutationFn: (payload: PlotCreatePayload) =>
      createPlot(propertyId!, payload),
    onSuccess: () => {
    ...
    onError: () =>
      toaster.create({ title: "Erro ao criar talhão.", type: "error" }),
  });

  const updatePlotMutation = useMutation({
    mutationFn: (payload: { id: number; data: PlotCreatePayload }) =>
      updatePlot(payload.id, payload.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["plots", propertyId] });
      plotFormDisclosure.onClose();
      toaster.create({
    ...
    onError: () =>
      toaster.create({ title: "Erro ao atualizar talhão.", type: "error" }),
  });
  ...
  const deletePlotMutation = useMutation({
    ...
    onError: () =>
      toaster.create({ title: "Erro ao remover talhão.", type: "error" }),
  });
          ...
          <Button
            ...
            onClick={() => {
              if (!canSubmit) {
                toaster.error({
                  title:
                    "Preencha todos os campos obrigatórios, incluindo um CNPJ válido.",
                });
                return;
              }
              onSubmit();
