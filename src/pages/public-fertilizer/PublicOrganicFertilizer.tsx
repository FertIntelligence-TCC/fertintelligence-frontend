...
import { DEFAULT_ORGANIC_FERTILIZER_FORM_STATE, OrganicFertilizerResponseDto, getFertilizerPhotoIds, fertilizerCommercialPriceResponseToForm, getOrganicMatterContent } from "@/interfaces/Fertilizer";
...
export default function PublicOrganicFertilizer() {
  return (
    <PublicFertilizerListBase<OrganicFertilizerResponseDto, typeof DEFAULT_ORGANIC_FERTILIZER_FORM_STATE>
      ...
      toForm={(item) => ({ ...DEFAULT_ORGANIC_FERTILIZER_FORM_STATE, nome: item.nome_adubo, c: String(item.c ?? 0), fotoIds: getFertilizerPhotoIds(item), ...fertilizerCommercialPriceResponseToForm(item), observacao: item.observacao ?? "", fonte: item.fonte ?? "", teorUmidade: String(item.teor_umidade ?? 0), teorMateriaOrganica: String(getOrganicMatterContent(item)), taxaMineralizacaoAno1: String(item.taxa_mineralizacao_ano_1 ?? 0), taxaMineralizacaoAno2: String(item.taxa_mineralizacao_ano_2 ?? 0), taxaMineralizacaoAno3: String(item.taxa_mineralizacao_ano_3 ?? 0), n: String(item.n ?? 0), p2o5: String(item.p2o5 ?? 0), k2o: String(item.k2o ?? 0), ca: String(item.ca ?? 0), mg: String(item.mg ?? 0), s: String(item.s ?? 0), b: String(item.b ?? 0), cu: String(item.cu ?? 0), fe: String(item.fe ?? 0), mn: String(item.mn ?? 0), mo: String(item.mo ?? 0), zn: String(item.zn ?? 0), publico: item.publico ? "sim" : "nao" })}
